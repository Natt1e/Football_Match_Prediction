...
def mainClass(index):
    ...
    while not workQueue.empty():
        try:
            ...
            print(num)
            newPage = changeHtmlPage(num)  # 手动模拟键盘操作，使网页进入目标页面
            ...
            if result == None:
                ...
                path = "./winRateData/" + str(num) + ".csv"
                with open(path, 'w', encoding='utf-8', newline='') as f:
                    writer = csv.writer(f)
                    if rate1.teamName != "0":
                        writer.writerow([rate1.teamName, rate1.homeRate, rate1.awayRate, round(rate1.difRate, 2)])
                    if rate2.teamName != "0":
                        writer.writerow([rate2.teamName, rate2.homeRate, rate2.awayRate, round(rate2.difRate, 2)])
                ...
                data = finalData(ele1.data1, ele1.data2, recentRate, recentGoal, leagueRate,
                                 leagueGoal, supportRate, result)
            if data:
                path = "./finalData/" + str(num) + ".csv"
                with open(path, 'w', encoding='utf-8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(
                        [round(data.fightRateDif, 4), round(data.fightGoalDif, 4), round(data.recentRateDif, 4),
                         round(data.recentGoalDif, 4), round(data.leagueRateDif, 4),
                         round(data.leagueGoalDif, 4),
                         round(data.supportDif, 4), round(data.result, 4)])
                print('第%s个页面分析完成,页面索引是%s' % (str(count), str(num)))
            else:
                print('第%s个页面为无效页面,页面索引是%s' % (str(count), str(num)))
            count += 1
            print("The process_id is %s, and the workQueue size is %s" % (str(Process_id), str(workQueue.qsize())))
        except Exception as e:
            print(Process_id, workQueue.qsize(), str(count), 'Error:', e)


def boss():
    for i in range(53410000, 53413000):
...
if __name__ == '__main__':
    ...
    with open('winRateDif2.csv', 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(headers)
    headers = ['fightRateDif', 'fightGoalDif', 'recentRateDif', 'recentGoalDif', 'leagueRateDif', 'leagueGoapDif',
               'supportDif', 'result']
    with open('finalData2.csv', 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(headers)
    '''
    ...
    jobs = []
    for i in range(20):
        jobs.append(gevent.spawn(mainClass, i))
    gevent.joinall(jobs)
    end = time.time()
